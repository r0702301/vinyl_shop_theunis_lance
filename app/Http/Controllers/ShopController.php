...
class ShopController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $result = compact('genres', 'records');     // $result = ['genres' => $genres, 'records' => $records]         // compact('records') is the same as ['records' => $records]
        Json::dump($result);                    // open http://vinyl_shop.test/shop?json
        return view('shop.index', $result);
    }

    public function alt_shop()
    {
        $genres = Genre::with('records')
            ->has('records')
            ->orderBy('name', 'asc')
            ->get();
        $result = compact('genres');           // compact('records') is the same as ['records' => $records]
        Json::dump($result);
